...
if (thid==0) opened
   ...
   if(PAROPT!=0) Parmapr(thid,0,1,0);
...
if (thid==0) opened
   ...
   if(PAROPT==0) Loadtgt(0,atmpstr1);
   if(PAROPT==1) Loadneur(0);
   if(PAROPT==2) Tspinit(0);
...
for(edp->cn=edp->cn0;edp->cn<=CYCLES;edp->cn++) opened
   ...
   for(ei=0;ei<NCORES;ei++) opened
      ...
      if (thid==ei) opened
         ...
         for(gi=0;gi<NNGUYS;gi++) for(di=0;di<(NEVENT*STAGES);di++) opened
            memcpy(&edp->frgenesa[gi][di],&edp->frgenesb[gi][di],sizeof(Dgx));
            memcpy(&edp->frgenesb[gi][di],&edp->frgenesc[gi][di],sizeof(Dgx));
            memcpy(&edp->frgenesc[gi][di],&edp->idgx,sizeof(Dgx));
         closed
         #endif
         if(PAROPT==1) Dsplneur(ei);
      closed
   closed
   // calc all_end ------------------------------------------------------------
   ...
   for(gi=gl;gi<gu;gi++) opened
      ...
      edp->motnr=0;
      // xxxx
      memset(&edp->platchgd[0],0,sizeof(int)*NPLATES);
      // development cycle
      ...
      for(as=ls;as<=us;as++) opened
         ...
         if ((edp->fdone==0)&&(as==edp->frz[edp->qf].ls)) opened
            ...
            edp->dhnrf=edp->dhnr;
            Forvxyz memcpy(&edp->pmpf [vx][vy][vz],&edp->pmap [vx][vy][vz],sizeof(int));
            edp->fset=1;
         closed
         if ((edp->fdone!=0)&&(as==ls)) opened
            ...
            edp->dhnr=edp->dhnrf;
            Forvxyz memcpy(&edp->pmap [vx][vy][vz],&edp->pmpf [vx][vy][vz],sizeof(int));
         closed
         //Disrupt (thid,gi,as);
         ...
         if((eval==1)&&(edp->fdone==0)) opened
            if(PAROPT==2) Parmapr(thid,gi,0,0);
         closed
         // Parameter fitness evaluation
         if(eval==1) opened
            if(PAROPT!=0) Parmapr(thid,gi,0,1);
            if(PAROPT==1) edp->fpopt=(float)Clcneur(thid,gi,ff,
               &edp->popa[pi]->guys[gy].mfit,edp->cfp);
            if(PAROPT==2) edp->fpopt=(float)Calctsp(thid,gi);
         closed
         #endif
         // fitness evaluation
         if(eval==1) opened
            cdl.x=0; cdl.y=0; cdl.z=0; cdu.x=GRIDX-1; cdu.y=GRIDY-1; cdu.z=GRIDZ-1;
            edp->fshad=Fitbox(thid,gi,&cdl,&cdu,&shas);
            if(PAROPT!=0) edp->fshad=edp->fpopt;
   ...
   if (thid==0) opened
      for(aa=NNGUYS-1;aa>=1;aa--) opened
         bb=edp->rndpmt[aa];
         memcpy(&edp->frgentmp    [0],&edp->frgenesc[aa][0],sizeof(Dgx)*(NEVENT*STAGES));
         memcpy(&edp->frgenesc[aa][0],&edp->frgenesc[bb][0],sizeof(Dgx)*(NEVENT*STAGES));
         memcpy(&edp->frgenesc[bb][0],&edp->frgentmp    [0],sizeof(Dgx)*(NEVENT*STAGES));
      closed
   closed
   #endif
   // calc master_end -------------------------------------------------------
...
if(PAROPT!=0) buflen0+=1*sizeof(int)*SZGRID;
if(PAROPT==1) buflen0+=1*sizeof(float)*NCAT*NEXTOT*784;
//if(PAROPT==2) buflen0+=1*sizeof(Coord)*PARSETSZ;
// sendrec_1
...
#if(TAGCHK==YA)
// frgenes
buflen3+=sizeof(Dgx)*(NNGUYS/NCORES+1)*(NEVENT*STAGES);
#endif
// calc
...
memset(&edp->iguy.gen,0,sizeof(int)*XGENSZ);
...
if(opt==0) opened
   ...
   closed
   if(PAROPT!=0) opened
      edp->envr.imap = new dimensa[GRIDX];
   closed
closed
...
#if(PAROPT!=0)
// grid blocks (to be moved elsewhere?)
int cb;
double tmpar[NPLATES*7]= opened CBOUND0 CBOUND1 closed ;
for(cb=0;cb <NPLATES;cb++) opened
   edp->platmr [cb]=     (int) tmpar[cb*7+0]; // activation ms
   edp->platbnd[cb][0].x=(int)(tmpar[cb*7+1]*1000*CBSCALE); 
   edp->platbnd[cb][1].x=(int)(tmpar[cb*7+2]*1000*CBSCALE);
   edp->platbnd[cb][0].y=(int)(tmpar[cb*7+3]*1000*CBSCALE); 
   edp->platbnd[cb][1].y=(int)(tmpar[cb*7+4]*1000*CBSCALE);
   edp->platbnd[cb][0].z=(int)(tmpar[cb*7+5]*1000*CBSCALE); 
   edp->platbnd[cb][1].z=(int)(tmpar[cb*7+6]*1000*CBSCALE);
   if(NDIMS==2) edp->platbnd[cb][0].z=0;
   if(NDIMS==2) edp->platbnd[cb][1].z=1;
closed

// parmap
Forvxyz edp->envr.imap[vx][vy][vz]=-1;
int parsubs[NPLATES*2]= opened PARSUBS closed; int lx,ly,lz,ux,uy,uz;
int mapcpn [NPLATES]= opened MAPCPN closed;
for(ii=0;ii<NPLATES;ii++) opened 
   edp->parsubt[ii][0]=parsubs[2*ii+0]; 
   edp->parsubt[ii][1]=parsubs[2*ii+1]; 
closed
cnt=0; for(cb=0;cb<NPLATES;cb++) opened
   lx=edp->platbnd[cb][0].x;  ux=edp->platbnd[cb][1].x;
   ly=edp->platbnd[cb][0].y;  uy=edp->platbnd[cb][1].y;
   lz=edp->platbnd[cb][0].z;  uz=edp->platbnd[cb][1].z;
   for(it=0;it<mapcpn[cb];it++) opened 
      aa=edp->parsubt[cb][0]; bb=edp->parsubt[cb][1];
      for(ii=aa;ii<bb;ii++) opened
         vx=lx; vy=ly; vz=lz;
         while(edp->envr.imap[vx][vy][vz]!=-1) opened
            vx=Rnd1(ux-lx)+lx; vy=Rnd1(uy-ly)+ly; vz=Rnd1(uz-lz)+lz; 
         closed 
         edp->envr.imap[vx][vy][vz]=ii; cnt++; 
      closed
   closed
   cvar=cvar;
closed
#if(DEBUG7==YA)
Forvxyz opened
   edp->envr.sgrd[vx][vy][vz]=edp->envr.imap[vx][vy][vz]; 
closed
Savegrid(thid,0,0);
#endif
#endif
...
// inits frgenes
for(gi=0;gi<NNGUYS;gi++) for(di=0;di<(NEVENT*STAGES);di++) opened
   memcpy(&edp->frgenesa[gi][di],&edp->idgx,sizeof(Dgx));
   memcpy(&edp->frgenesb[gi][di],&edp->idgx,sizeof(Dgx));
   memcpy(&edp->frgenesc[gi][di],&edp->idgx,sizeof(Dgx));
closed
// inits hstgenes
for(ii=0;ii<10000;ii++) opened
   memcpy(&edp->hstgenes[ii].dgx,&edp->idgx,sizeof(Dgx));
...
// inits taghst
for(ri=0;ri<6400;ri++) for(ci=0;ci<STAGES+2;ci++) edp->taghst[ri][ci]=0;
#endif
...
#if(PAROPT==1)
int lr,ndsplr[LRSTOT]=NDSPLR;
for(lr=0;lr<LRSTOT;lr++) edp->ndsplr[lr]=ndsplr[lr];
#endif
