...
typedef struct opened
   dimensa *etis,*cdis,*cdtg,*imap,*sgrd;
   float neurtot[NCAT][NEXTOT][784];
   float neurspl[NCAT][NEXSPL][784];
   //Coord citpos[PARSETSZ];
closed  Envir;
...
typedef struct opened
   ...
   #if(TAGCHK==YA)
   int    hgnr,tcount; float taghst[320][STAGES+2];
   Dgx    frgenesa[NNGUYS][NEVENT*STAGES];//int ancesta[NNGUYS],ftevala[NNGUYS];
   Dgx    frgenesb[NNGUYS][NEVENT*STAGES];//int ancestb[NNGUYS],ftevalb[NNGUYS];
   Dgx    frgenesc[NNGUYS][NEVENT*STAGES];//int ancestc[NNGUYS],ftevalc[NNGUYS];
   Dgx    frgentmp[NEVENT*STAGES]; Dgf hstgenes[50000];
   #endif
   // Aux-morphogens  -------------------------------------------------------
   ...
   // PAROPT ----------------------------------------------------------------
   //#if(PAROPT!=0)
   float  fshad,fmfit,fpopt,parset[PARSETSZ]; //float pardlt[PARSETSZ];
   int    parcnt[PARSETSZ];
   int    pmap[GRIDX][GRIDY][GRIDZ]; // current status
   int    pmpf[GRIDX][GRIDY][GRIDZ]; // platform status
   //#endif
   int    mothers[1000][2],motnr;
   int    ndsplr[LRSTOT],neurout[NDSTOT]; // neural network nodes
   int    whtsmat[NDSTOT][NDSTOT]; // neural network connections
   float  outmat[NCAT][NCAT*NEXSPL],outar[NCAT*NEXSPL],mscar[NCAT*NEXSPL];
   // --- TSP ---------------------------------------------------------------
   //Parel  citlist[PARSETSZ]; 
   // --- Other -------------------------------------------------------------
   char   astr[50]; int cnta,curpt,ctr; float exbst;
   int    platmr[NPLATES],platchgd[NPLATES],parsubt[NPLATES][2];
   float  ordar[NCAT],gresid;
   Coord  ext[2],platbnd[NPLATES][2]; FILE *cfp;
...
float Fitbox (int thid,int gi,Coord *cdl,Coord *cdu,float *shas);
// File 3: neural computation
void Smooth3d (int thid,int gi,Coord *ext0,Coord *ext1,int nhs);
void Loadneur (int thid);
void Dsplneur (int thid);
void Parlr (int thid,int oa,int ob,int da,int db,int *ii);
void Parmapr (int thid,int gi,int aa,int oo);
void Parexcl (int thid,int as);
float Clcneur (int thid,int gi,int ff,float *mfit,FILE *fp);
void  Calclr (int thid,int ua,int ub,int la,int lb);
void  Drawex (int thid,int gi);
void  Tspinit (int thid);
float Calctsp (int thid,int gi);
void Detplate (int thid,int *pi,int vx,int vy,int vz);
// File 4
